...
- 추가 요청 반영: 단순 난이도 상향이 아니라 문항 퀄리티 개선 작업 시작.
- 수정 메모: 공부 페이지 출제를 교사용 로컬 문항 중심으로 강화하고, 문항별 해설/오개념/교사 메모를 표시하도록 확장.

- 추가 요청 반영: 정답 시 적응형 난이도 상승, 오답/패스 시 완화, 최근 유형 반복 억제 로직 작업 시작.
- 검증 메모: /practice/basic 에서 로그 문제 정답 후 바로 CORE 연속성 문제로 상승하는 것 확인. 직전 유형 반복 없이 다른 포커스로 전환됨.
